...
| 进入微信模式 | `mcp__wecom-aibot__enter_headless_mode` | cc_id, project_dir, mode |
...
**必须使用 AskUserQuestion 让用户选择模式**：

```
使用 AskUserQuestion 让用户选择模式:
    question: "请选择微信消息接收模式"
    options:
      - "Channel模式 - 消息自动推送（需要 claude --channel 启动）"
      - "HTTP模式 - 需要轮询获取消息（兼容模式）"
```

**模式说明**：
- **Channel模式**：SSE 推送，微信消息自动唤醒 Agent。需要 Claude Code 用 `--channel` 参数启动。
- **HTTP模式**：轮询模式，Agent 需调用 `get_pending_messages` 和 `heartbeat_check`。兼容所有环境。

**如果用户选择 Channel 模式但没有用 `--channel` 启动**：
- 提示用户："Channel 模式需要用 `claude --channel` 启动。当前会话不支持，请退出后重新启动，或选择 HTTP 模式。"
- 让用户重新选择
...
4. 使用 AskUserQuestion 让用户选择模式（channel 或 http）
5. 调用 enter_headless_mode(cc_id, robot_id=确定的机器人名称, mode=用户选择的模式)
```
...
智能体生成 cc_id，格式为 `${项目名}-${序号}`（如 `wecom-aibot-mcp-1`）。

**必须使用 AskUserQuestion 让用户确认 cc_id**：

```
使用 AskUserQuestion 让用户确认名称:
    question: "请确认会话名称（用于在微信消息中标识）"
    options:
      - "<生成的cc_id>"
      - "自定义名称"
```

如果用户选择"自定义名称"，再次用 AskUserQuestion 让用户输入：
```
question: "请输入自定义会话名称"
```
...
**参数说明**：
- `cc_id`: **必填**，CC唯一标识（智能体生成，如 `wecom-aibot-mcp-1`）
- `robot_id`: **必填**（多机器人场景），指定机器人名称
- `mode`: **必填**，运行模式：
  - `channel`：SSE 推送，微信消息自动唤醒 Agent（需要 `--channel` 启动参数）
