...
调用 `mcp__wecom-aibot__list_robots`：
...
**必须用 AskUserQuestion 让用户选择**：

- **Channel 模式**：消息自动推送，需要 `claude --channels server:wecom-aibot-channel` 启动
- **HTTP 模式**：需要轮询，兼容所有环境

#### 1.4 调用 enter_headless_mode
...
**返回字段**：
- `status: 'entered'` → 进入成功
- `status: 'ccid_conflict'` → ccId 冲突，提示用户改名后重新调用
...
3. 写入 `.claude/wecom-aibot.json`：
   ```json
   {
     "ccId": "xxx",
     "robotName": "xxx",
     "heartbeatJobId": "abc12345",  // 添加此字段
     "wechatMode": true,
     "autoApprove": true
   }
   ```
...
3. 发送结果：`mcp__wecom-aibot__send_message("【完成】...")`
4. 继续轮询（HTTP 模式）
